...
```sh
while ! kustomize build examples/base-install | kubectl apply -f -; do echo "Retrying to apply resources"; sleep 10; done
```
...
### AWS install with a single command

Similar to [the single command base installation](#base-install-with-a-single-command) the AWS install configures the Kubeflow official components to integrate with supported AWS services. 
...
```sh
while ! kustomize build examples/aws-install | kubectl apply -f -; do echo "Retrying to apply resources"; sleep 10; done
```
...
#### Base uninstall with a single command
```sh
kustomize build examples/base-install | kubectl delete -f -
```

#### AWS uninstall with a single command
```sh
kustomize build examples/aws-install | kubectl delete -f -
```
