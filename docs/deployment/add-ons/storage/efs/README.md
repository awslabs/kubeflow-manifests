...
```
cd kubeflow-manifests/docs/deployment/add-ons/examples/storage
```
...
1. Use the `$file_system_id` you recorded in section 4 above or use the AWS Console to get the filesystem id of the EFS volume you want to use. Now edit the last line of the sample/pv.yaml file to specify the `volumeHandle` field to point to your EFS filesystem. Replace `$file_system_id` if it is not already set. 
```
file_system_id=$file_system_id yq e '.spec.csi.volumeHandle = env(file_system_id)' -i efs/static-provisioning/pv.yaml
```
...
### 6.4 Delete the EFS mount targets, filesystem and security group
