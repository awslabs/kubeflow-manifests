...
def create_AMP_workspace(region):
    ...
    workspace_id = amp_client.create_workspace(alias="test_AMP_workspace")[
        "workspaceId"
    ]
...
def set_up_prometheus_for_AMP(cluster_name, region):
    ...
    create_AMP_service_account_command = (
    ...
    create_namespace_command = (
        f"kubectl create namespace " f"{PROMETHEUS_NAMESPACE}".split()
    )
    subprocess.call(create_namespace_command)

    kustomize_build_command = (
        f"kustomize build " f"{prometheus_yaml_files_directory}".split()
    )
...
def set_up_prometheus_port_forwarding():
    ...
    def get_active_prometheus_pod_name():
        get_pod_name_command = (
            f"kubectl get pods " f"--namespace={PROMETHEUS_NAMESPACE}".split()
        )
        print(" ".join(get_pod_name_command))
        pod_list = subprocess.check_output(
            get_pod_name_command, encoding="utf-8"
        ).split("\n")[1:]
    ...
    prometheus_pod_name = wait_for(
        get_active_prometheus_pod_name, timeout=90, interval=30
    )
    if None == prometheus_pod_name:
        raise ValueError("Prometheus Pod Not Running.")

    set_up_port_forwarding_command = (
...
def get_prometheus_query_results(prometheus_query=default_prometheus_query):
    ...
    prometheus_query_results = subprocess.check_output(
        prometheus_curl_command, encoding="utf-8"
    )
...
def check_AMP_connects_to_prometheus(
    region, workspace_id, expected_value, prometheus_query=default_prometheus_query
):
    ...
    # ~/.aws/credentials.
    AMP_awscurl_command = (
        f"awscurl --region {region} --service aps "
...
def delete_AMP_resources(cluster_name, region, workspace_id):
    ...
    delete_AMP_service_account_command = (
        f"eksctl delete iamserviceaccount "
    ...
    raise AssertionError(
        "The workspace with id", workspace_id, "is in the {workspace_status} state."
    )
