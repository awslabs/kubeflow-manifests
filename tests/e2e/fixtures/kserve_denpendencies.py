...
from e2e.fixtures.cluster import cluster, create_iam_service_account, associate_iam_oidc_provider, delete_iam_service_account
from e2e.utils.utils import rand_name, write_yaml_file, exec_shell, wait_for, load_yaml_file, kubectl_apply
...
def clone_tensorflow_serving():
    tensorfow_serving_path = "../../serving"
    if not os.path.isdir(tensorfow_serving_path):
...
@pytest.fixture(scope="class")
def kserve_iam_service_account(
    metadata, cluster, region, request
):
    metadata_key = "aws-sa"
...
def kserve_secret(metadata, cluster, region, request):
    ...
    def on_create():
        ...
        #patch secret to IRSA in prifle namespace
        exec_shell("kubectl patch serviceaccount aws-sa -n kubeflow-example-com -p '{\"secrets\": [{\"name\": \"aws-secret\"}]}'")
...
def kserve_inference_service(metadata, cluster, region, request):
    ...
    def on_create():
        ...
        inference_config["spec"]["predictor"]["model"]["storageUri"] = f"s3://{bucket_name}/saved_model_half_plus_two/"
...
def wait_for_inference(cluster_name: str, region: str):
        ...
        def callback():
            inference_service = get_inference_service(cluster_name, region, namespace="kubeflow-example-com")
