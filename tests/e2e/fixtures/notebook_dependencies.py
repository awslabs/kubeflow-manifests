...
def notebook_server(region, metadata, request, framework_name, image_name, ipynb_notebook_file):
    ...
    notebook_file_path = (
        TO_ROOT_PATH
        + "tests/e2e/resources/notebooks/" + ipynb_notebook_file 
    )

    notebook_server_pvc_spec_file = (
        TO_ROOT_PATH
        + "tests/e2e/resources/custom-resource-templates/notebook-pvc.yaml"
    )
    notebook_server_spec_file = (
        TO_ROOT_PATH
        + "tests/e2e/resources/custom-resource-templates/notebook-crd.yaml"
    )
    replacements = {"REGION": region, "NOTEBOOK_NAME": notebook_name, "NAMESPACE": DEFAULT_USER_NAMESPACE, "IMAGE": image_name}
